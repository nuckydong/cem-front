...
tinymce.PluginManager.add('tyRadio', function (ed) {
  var number = 1;
  ...
  function renderComponent(options) {
    ...
    for (let i = 0; i < opt.selectList.length; i++) {
      ...
      str.push('<label style="' + elst + '" data-el="item-src-select"  ><input  style="cursor: not-allowed" type="radio" name="' + opt.id + '"  '+(opt.selectList[i].value==opt.value?"checked":"")+'/>' + opt.selectList[i].label + '</label>');
    }
