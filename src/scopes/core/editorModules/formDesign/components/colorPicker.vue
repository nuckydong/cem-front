<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    <!-- 颜色显示小方块 -->
    <ta-input  class="chooseColor" @blur="openStatus=false" @focus="openStatus = !disabled" :value="`${showColor}`" >
      <ta-icon slot="addonAfter" type="eye"  v-bind:style="`color: ${showColor};background-color: ${showColor};font-size:22px;`"></ta-icon>
    </ta-input>
    <div class="colorBtn"
      v-bind:style="`background-color: ${showColor};display:none;`"
...
.m-colorPicker{
  ...
  .box{
    z-index: 9999;
    position: absolute; width: 226px; background: #fff; border: 1px solid #ddd; visibility: hidden; border-radius: 2px; margin-top: 2px; padding: 10px; padding-bottom: 5px; box-shadow: 0 0 5px rgba(0,0,0,.15); opacity: 0; transition: all .3s ease;
