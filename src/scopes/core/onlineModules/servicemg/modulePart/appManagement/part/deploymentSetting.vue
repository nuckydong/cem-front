  ...
  <div style="height: 100%">
    ...
    <ta-table :columns="columns" style="width:100%;" :pagination='false' rowKey="appId" :data-source="gridData" :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}">
      <span slot="effective" slot-scope="text">
          <ta-tag v-if="text == '1'" color="green" class="no-cursor">{{CollectionLabel('EFFECTIVE', text)}}</ta-tag>
          <ta-tag v-else color="red" class="no-cursor">{{CollectionLabel('EFFECTIVE', text)}}</ta-tag>
      </span>
      <template slot="operation" slot-scope="text, record, index">
        <a @click="editVisible = true; rowData = record; editType = record.serverType">编辑</a>
        <ta-divider type="vertical"/>
        <ta-popconfirm
          title="确认删除该应用?"
          okText="确认"
          cancelText="取消"
          @confirm="addSuccess(record)">
          <a>删除</a>
        </ta-popconfirm>
      </template>
    </ta-table>
...
const columns = [
  ...
  { title: '有效标识', dataIndex: 'effective', width: 120, scopedSlots: {customRender: 'effective'} },
