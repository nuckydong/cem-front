  ...
  <div class="fit">
    <ta-border-layout :layout="{header:'55px'}" :centerCfg="{showBar: true}">
            ...
            <ta-col :span="row.col.span">
              <ta-table :columns="orgColumns"
                ...
                <template slot="action" slot-scope="record">
                  <ta-popconfirm title="确定要移除该管理员的组织权限及其所有子权限吗?" okText="确认" cancelText="取消"
                                 @confirm="() => fnAdminOrgDelete(record)">
      ...
      <ta-modal :visible="authority.visible" :centered="true" :destroyOnClose="true"
        ...
        <!--</div>-->
        <!--</template>-->
        <admin-authority-mg :item="item" @modalCancel="fnReLoadAuthority"></admin-authority-mg>
      </ta-modal>
      <!--end 权限管理模态框加载-->
    </ta-border-layout>
  </div>
...
export default {
  ...
  methods: {
    ...
    fnLoadDefault () {
      ...
      $api.queryOrgPermission(self, data, (data) => {
    ...
    fnAdminOrgDelete (record) {
      ...
      $api.removeOrgPermission(self, data, (data) => {
